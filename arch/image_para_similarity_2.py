...
def clean_text(text):
    """
    Remove stopwords, punctuation, digits, and extra whitespace from a string.
    """
    text = text.lower()
...
def process_images_and_paragraphs(image_files, paragraphs, model, preprocess, device, output_dir):
    ...
    # Tokenize all paragraphs at once
    tokenized_texts = tokenize_with_truncate(paragraphs, device)
    ...
    for fileName in image_files:
        ...
        ranked = sorted(zip(paragraphs, sims), key=lambda x: x[1], reverse=True)

        # Print top-5 matches
        for para, score in ranked[:5]:
            print(f"→ {score:.4f} | {para[:80]}...")

        # Save all results to CSV
        outputfile = os.path.join(output_dir, os.path.basename(fileName) + ".csv")
        with open(outputfile, "w", newline="") as csv_file:
            writer = csv.writer(csv_file)
            writer.writerow(["Paragraph", "Similarity"])
