...
def process_images_and_paragraphs(image_files, paragraphs, model, preprocess, device, output_dir):
    ...
    for fileName in image_files:
        ...
        # Attach metadata back: (page, para_index, original_text, cleaned_text, similarity)
        ranked = sorted(
            [(para[0], para[1], para[2], para[3], sims[i]) for i, para in enumerate(paragraphs)],
            key=lambda x: x[4],
            reverse=True
        )

        # Print top-5 with original paragraph text
        for page, para_index, orig_text, _, score in ranked[:5]:
            preview = orig_text.replace("\n", " ")[:80]  # short preview
            print(f"→ {score:.4f} | Page {page}, Paragraph {para_index}: {preview}...")

        # Save to CSV
        outputfile = os.path.join(output_dir, os.path.basename(fileName) + ".csv")
        with open(outputfile, "w", newline="") as csv_file:
            writer = csv.writer(csv_file)
            writer.writerow(["Page", "Paragraph", "Original Text", "Cleaned Text", "Similarity"])
